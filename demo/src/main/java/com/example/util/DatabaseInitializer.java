...
public class DatabaseInitializer {
    ...
    private static final String CREATE_TABLE_MAJORS = """
    ...
    /**
     * Chèn hoặc reset tài khoản admin mặc định (username: admin / password:
     * admin123).
     */
    private static final String INSERT_DEFAULT_ADMIN = """
            ...
            ON CONFLICT(username) DO UPDATE SET password = 'admin123', active = 1
            """;

    /**
     * Chèn hoặc reset tài khoản thủ thư mặc định (username: thuthu / password:
     * thuthu123).
     */
    private static final String INSERT_DEFAULT_LIBRARIAN = """
            INSERT INTO users (username, password, full_name, role, active)
            VALUES ('thuthu', 'thuthu123', 'Nguyễn Thị Thu', 'LIBRARIAN', 1)
            ON CONFLICT(username) DO UPDATE SET password = 'thuthu123', active = 1
            """;
    ...
    private static void createTables(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            stmt.execute(CREATE_TABLE_MAJORS);
            ...
            // Migration: tách Khối Ngành thành bảng riêng.
            // DB cũ từng lưu major trực tiếp trong categories/books, nên giữ cột cũ để tương thích
            // nhưng major_id mới là dữ liệu chuẩn.
            try { stmt.execute("ALTER TABLE categories ADD COLUMN major_id INTEGER"); } catch (SQLException ignored) {}
            try { stmt.execute("ALTER TABLE categories ADD COLUMN major TEXT DEFAULT 'Khác'"); } catch (SQLException ignored) {}
            ...
            for (String major : defaultMajors) {
                try (PreparedStatement ps = conn.prepareStatement("INSERT OR IGNORE INTO majors(name) VALUES(?)")) {
                    ps.setString(1, major); ps.executeUpdate();
                }
            }

            // Chuyển dữ liệu major cũ -> major_id.
            try {
                stmt.execute("UPDATE categories SET major_id=(SELECT id FROM majors WHERE LOWER(TRIM(majors.name))=LOWER(TRIM(categories.major))) WHERE major IS NOT NULL AND TRIM(major)!=''");
            } catch (SQLException ignored) {}

            // Migration: đồng bộ các thể loại hiện có trong bảng books vào bảng categories
            // nếu chưa có
            try {
                stmt.execute(
                        "INSERT OR IGNORE INTO categories (name) SELECT DISTINCT TRIM(category) FROM books WHERE category IS NOT NULL AND TRIM(category) != ''");
            } catch (SQLException ignored) {
            }

            // Gán khối ngành cho các thể loại mặc định. Một khối ngành có thể có nhiều thể loại.
            String[][] categoryMajorMap = {
                    {"Thiếu nhi", "Thiếu Nhi & Giáo Dục"}, {"Ngoại ngữ", "Thiếu Nhi & Giáo Dục"},
                    {"Văn học", "Văn Học & Nghệ Thuật"}, {"Nghệ thuật", "Văn Học & Nghệ Thuật"},
                    {"Công nghệ", "Công Nghệ & Kỹ Thuật"}, {"Triết học", "Khoa Học Xã Hội & Nhân Văn"},
                    {"Lịch sử", "Khoa Học Xã Hội & Nhân Văn"}, {"Kỹ năng sống", "Kỹ Năng & Phát Triển Bản Thân"},
                    {"Khoa học", "Khoa Học Tự Nhiên"}, {"Kinh tế", "Kinh Tế & Quản Trị"}
            };
            for (String[] pair : categoryMajorMap) {
                try (PreparedStatement ps = conn.prepareStatement("UPDATE categories SET major_id=(SELECT id FROM majors WHERE name=?), major=? WHERE LOWER(TRIM(name))=LOWER(TRIM(?))")) {
                    ps.setString(1, pair[1]); ps.setString(2, pair[1]); ps.setString(3, pair[0]); ps.executeUpdate();
                }
            }
            try {
                stmt.execute("UPDATE categories SET major_id=(SELECT id FROM majors WHERE name='Khác'), major='Khác' WHERE major_id IS NULL");
            } catch (SQLException ignored) {}

            // Migration: đồng bộ các tác giả hiện có trong bảng books vào bảng authors nếu
            // chưa có
            try {
                stmt.execute(
                        "INSERT OR IGNORE INTO authors (name) SELECT DISTINCT TRIM(author) FROM books WHERE author IS NOT NULL AND TRIM(author) != ''");
            } catch (SQLException ignored) {
            }
    ...
    private static void insertDefaultData(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            ...
            // Gán Khối Ngành cho các thể loại mặc định sau khi chúng được tạo.
            String[][] defaultCategoryMajors = {
                    {"Thiếu nhi", "Thiếu Nhi & Giáo Dục"}, {"Ngoại ngữ", "Thiếu Nhi & Giáo Dục"},
                    {"Văn học", "Văn Học & Nghệ Thuật"}, {"Nghệ thuật", "Văn Học & Nghệ Thuật"},
                    {"Công nghệ", "Công Nghệ & Kỹ Thuật"}, {"Triết học", "Khoa Học Xã Hội & Nhân Văn"},
                    {"Lịch sử", "Khoa Học Xã Hội & Nhân Văn"}, {"Kỹ năng sống", "Kỹ Năng & Phát Triển Bản Thân"},
            ...
            for (String[] pair : defaultCategoryMajors) {
                try (PreparedStatement ps = conn.prepareStatement("UPDATE categories SET major_id=(SELECT id FROM majors WHERE name=?), major=? WHERE name=?")) {
                    ps.setString(1, pair[1]); ps.setString(2, pair[1]); ps.setString(3, pair[0]); ps.executeUpdate();
                }
            }
